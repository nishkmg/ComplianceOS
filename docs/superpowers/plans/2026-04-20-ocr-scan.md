...
**Goal:** Upload invoice image/PDF → extract line items + GST → create draft invoice

**Architecture:**
- Tesseract.js (client-side, no API key, WASM-based) for OCR
- Next.js API route for file upload → stores to `/uploads` (S3-ready interface)
- Rules-based Indian invoice parser (regex patterns for GSTIN, invoice number, line items)
- Reuse existing `createInvoice` command handler for draft invoice creation
- OCR results stored in `ocr_scan_results` table for audit trail + re-editing

**Tech Stack:** Tesseract.js, Next.js API routes, Drizzle ORM, Zod, tRPC
...
packages/db/src/schema/
  ocr-scan.ts              # ocr_scan_results table

packages/server/src/services/
  ocr-processor.ts         # Tesseract.js wrapper (server-side)
  invoice-parser.ts        # Rules-based Indian invoice parser
  file-upload.ts           # File upload to /uploads (S3-ready)

packages/server/src/routers/
  ocr-scan.ts              # tRPC router: upload, parse, get, delete
