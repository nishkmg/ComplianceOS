...
- Snapshot/PDF fixtures in `packages/server/src/__tests__/pdf-snapshots` — PDF output changes break them; review deliberately. **PDF tests hash EXTRACTED TEXT (`pdfTextDigest`), not raw bytes** — pdfkit's font-subset embedding (glyph ids, object order, /FontName tags, /ID) is nondeterministic, so byte hashes can never be stable. **@react-pdf/renderer 4.x renders BLANK PDFs** when no real font file is registered (standard Helvetica is unsupported) AND when `Font.registerHyphenationCallback(() => [])` is called — both silent; Inter/Noto TTFs are bundled in `packages/server/assets/fonts` and registered from local paths in `pdf-engine.tsx`. Run the pdf suite under node 20 (CI) — text digests are node-agnostic, byte hashes were not.
...
- **No Supabase runtime code remains** (Aug 2026): `lib/supabase-rest.ts` + `lib/supabase-realtime.ts` deleted; realtime-provider is a 15s polling bus (synthetic UPDATE events fan out to trpc invalidation listeners); `/api/health` + ITR PDF route check the local DB; `@supabase/realtime-js` dep removed.
- **API surface converged on tRPC (Aug 2026)** — legacy `/api/*` handlers for accounts, coa, employees, fiscal-years, gst/ledger, gst/payments, gst/returns, inventory/*, invoices, journal/entries, payments, payroll/runs were deleted after their pages migrated to routers. Kept: `auth/*`, `contact`, `health`, `gst/returns/[id]/pdf`, `itr/returns/[id]/pdf`, `trpc`, `upload`, `uploads/[...path]`, and **`onboarding`** (working wizard — the one deliberate REST exception; `submitStep` in `lib/mock-mutation.ts` is a REAL fetch despite the name). Pages that still fetch `/api/*` are bugs — migrate them.
- **tRPC wire format here is v11 flat**: POST batch bodies are `{"0":{...}}` (NO `json` key), GET queries need `?input={"0":{...}}` — curl/Playwright probes that wrap input in `{"0":{"json":...}}` get empty-input validation errors and mislead (checked itrReturns + gstr routers Aug 2026).
- **`event_type` enum keeps missing values**: GST/ITR generate+PDF flows append events the enum predates — 3 separate ALTERs so far (0038: gstr1/2b_generated, 0039: itr_return_*+itr_pdf_generated). Before claiming any event-flow works, dump `SELECT unnest(enum_range(NULL::event_type))` and diff against `appendEvent(` call sites. Also: `appendEvent` actor must be a REAL user UUID — passing `"system"` crashes the insert (fixed in generate-itr-pdf; route passes `token.sub`).
- **`gst_tax_type` enum grew `interest`/`penalty` (0045)**: gst-cash-balance projector maps interestAmount/penaltyAmount to their own tax buckets (was `igst`, inflating the IGST cash balance). Same trap class as event_type — the 0038 file was lost in the migration repair (recreated Aug 2026); **drizzle-kit migrate rewrites `meta/_journal.json` from the files present in the folder and DROPS journal entries whose SQL files are missing (0042/0043)** — never rely on it to apply; psql + manual `__drizzle_migrations` rows (`created_at` = journal `when`, ms) is the safe local path. `loginAttempts` export in `db/src/index.ts:83` is orphaned (schema file gone) — `pnpm typecheck` fails on it at HEAD.
- **ITR income pipeline bootstraps from books**: `itrComputation.computeIncomeFromBooks` aggregates posted journal lines (Revenue−Expense within FY window from fiscal_years) and appends `income_computed` + runs `itrAnnualIncomeProjector` INLINE — the projector only fires on events, computeIncome requires the projection, and the worker may be absent on serverless; the inline run is required, not a hack. Create flow chains computeFromBooks → computeTax("new") → generate; Regenerate runs the whole chain.
- **Demo E2E fixture tenant**: local DB user `rpt-test-47b0d6@example.com` / `fixture123` (id 192af6eb, tenant 47b0d60b) holds the 2-invoice GST/ITR fixture (₹20,000 sales in FY 2026-27) — the seeded `demo@complianceos.test` tenant (00000000-…-0001) has no books, so ITR shows honest ₹0. Used for browser E2E; password is local-only.
- **OCR is pluggable** (`packages/server/src/services/ocr-engine.ts`): `OCR_PROVIDER=tesseract` (default, Tesseract.js + regex parser, offline) or `llm` (OpenAI-compatible vision endpoint; needs `OCR_LLM_API_KEY`, falls back to tesseract when missing). PDFs are always OCR'd locally then structured by the LLM (vision APIs reject PDFs). Config: `OCR_LLM_BASE_URL` (Gemini: `https://generativelanguage.googleapis.com/v1beta/openai`), `OCR_LLM_MODEL` (default gpt-4o-mini). Router (`ocr-scan.ts`) consumes `processScan(fileUrl, scanType)` — don't bypass it.
- **`page-audit.mjs` flags fake-success** (success toast with no fetch/useQuery/useMutation in the file) — count is 0; keep it that way. Onboarding `step-*` wizard is live; `screen-*` onboarding files were deleted (dead code).
...
- **fiscal-year actions are real**: POST /api/fiscal-years (create) + /api/fiscal-years/[id]/close exist (commands exported from @complianceos/server); the settings page wires Close FY to CloseFiscalYearDialog. inventory tRPC summary computes lowStock/outOfStock/hsnCompliance from valuation + product HSN counts (was a 15% fabrication).
- **Page coverage ledger**: `node scripts/page-audit.mjs` rewrites `PAGE-AUDIT.md/.json` (per-route data source, fabrication flags, PageHeader, banned utilities, a11y-sweep + nav coverage). Use it before claiming "all pages covered". **A11y sweep** (`apps/web/e2e/a11y.spec.ts`) now covers 57 app + 8 public routes; local run: prod server on :3100 with `AUTH_URL=http://localhost:3100` (else NextAuth redirects to :3000 and the login POST fails with connection refused) + `pnpm --filter @complianceos/web exec playwright test e2e/a11y.spec.ts --project=chromium` (webkit/firefox not installed locally; playwright config `testIgnore` skips macOS `._*` junk). Dark cards need `text-amber-bright`/`text-white` headings (text-amber + text-light fail 4.5:1 on `bg-dark`) — the a11y sweep is the gate.
...
- **Marketing display scale (Aug 2026)**: `.text-hero` (clamp 2.25rem->3.5rem) for full-width h1s, `.text-hero-split` (clamp 1.75rem->2.75rem) for split-hero columns — the fixed `text-marketing-hero` (64px) token wraps to 3-5 lines in split heroes and 4-6 lines at 480px. Verify hero line counts after any marketing hero change (2 desktop / 3 mobile max, measured via getBoundingClientRect/lineHeight).
- **Marketing system (Aug 2026, ledger/print-modern)**: shared components in `components/marketing/` — `MarketingButton` (only button), `SectionHeader` (eyebrow budget), `CtaBand` (one "Start Free" label), `BrowserFrame` (real chrome + `/images/marketing/*.png` real app screenshots), `CapabilityTicker`, `Reveal` (motion/react scroll reveal). Rules: max 1 eyebrow per 3 sections, zero em-dashes, zero fabricated stats, no three-equal-card rows, mono uppercase labels are the aesthetic (data labels ≠ eyebrows). **Axe race gotcha**: hero load animations MUST NOT fade opacity — automated contrast scanners catch mid-fade blended colors and fail the a11y gate; translate-only (y) motion is safe. The old fake-logo wall (`logo1-5.png`) and dead components (feature-card/pricing-card/testimonial-card/faq-item/team-card/section-label/mobile-nav) were removed.
- **Button system (Aug 2026): one 36px/4px language.** Component `Button` (h-9 default / h-8 sm / h-10 lg, `rounded-sm`, gap-1.5, `[&_svg]:size-4` — every icon child renders 16px regardless of stray classes), global `.btn` (fixed `height: 2.25rem` — NOTE: `.btn`'s unlayered padding beats `py-*` utilities, so those are dead on marketing CTAs), inputs/selects h-9/rounded-sm, `--radius-sm` = 4px. Icon sizing: lucide's `size` prop wins over `text-*` classes (they're no-ops on SVGs) — pass explicit `size={14|16|20}` or rely on `[&_svg]:size-4`.
- **tailwind-merge must know the custom text tokens**: `lib/utils.ts` extends the font-size class group with text-ui-*/mono-*/display-* — without it, `text-white` gets dropped next to `text-ui-sm` (buttons render dark-on-amber). If new text-size tokens are added, extend that list too.
...
- The RLS suite passes 14/14 (real enforcement: `app_test` role + FORCE RLS + session `SET app.tenant_id` — note `SET x = $1` is invalid, use `unsafe` literals, and postgres-js 3.4 has no `onconnect`).
...
- **Prod (arthvahi.in) = Vercel serverless; DB = Supabase project `jjffitzswjizxcsdhtjn`** (Aug 2026): the working pooler host is `aws-1-ap-northeast-1.pooler.supabase.com:6543` (user `postgres.<ref>`), NOT `aws-0-ap-south-1` — a wrong pooler region returns `tenant or user not found`. The direct host `db.<ref>.supabase.co` is **IPv6-only (no A record)** — unusable from Vercel/Lambda (IPv4-only egress). `?sslmode=require` works for the app (postgres-js) but pg/drizzle-kit treats it as verify-full and fails on Supabase's cert — for manual migrations use a script with `ssl: { rejectUnauthorized: false }` or `PGSSLMODE=no-verify`.
- **Prod drizzle journal is baselined 30/30** (old drizzle table has `created_at bigint`, new installs use timestamptz — do not assume shape). Run `db:migrate` against prod only with the correct pooler URL; the app never migrates (Vercel build doesn't).
- **Drizzle migrator decides pending by `created_at` vs journal `when`** — the dialect checks `lastDbMigration.created_at < migration.folderMillis`, so a hand-baselined journal row MUST store epoch **milliseconds** in `created_at` (epoch seconds silently re-applies every migration). When hand-adding journal entries, use `Date.now()` (ms) for `when` AND for any manual `created_at` inserts.
- **GitHub Actions is DISABLED at the repo level** (Settings → Actions → General) — CI runs only after it's enabled; `gh api .../actions/permissions` returned `enabled=false`. No run appears on push until then.
- **OCR scans are worker-queued**: `upload` mutation inserts `status='processing'`; the projector worker drains via `FOR UPDATE SKIP LOCKED` (durable, crash-safe). No fire-and-forget in the router. On Vercel there is no worker — `PROJECTOR_URL` unset makes `/api/health` report `not_configured` (healthy), and OCR stays `processing` until a worker (PM2 or CF Worker) exists.
