...
- **Known-failing server tests** (pre-existing, surface once a working DB is used): `gst-ledger-service.test.ts` mocks a Prisma-style `db.query.X.findMany` API against the Drizzle service (mock debt), `income-computation-service.test.ts` numeric-string + fixtures, `payroll-integration` fixture FK ordering. The RLS suite now passes 14/14 (real enforcement: `app_test` role + FORCE RLS + session `SET app.tenant_id` — note `SET x = $1` is invalid, use `unsafe` literals, and postgres-js 3.4 has no `onconnect`).
