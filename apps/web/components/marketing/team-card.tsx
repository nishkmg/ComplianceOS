...
export function TeamCard({ name, role, bio }) {
  return (
    <div className="bg-surface border border-border-subtle rounded-lg p-7">
      ...
      <h3 className="font-ui text-ui-lg font-medium text-dark">{name}</h3>
      <p className="font-ui text-ui-md text-light mb-3">{role}</p>
      <p className="font-ui text-ui-md text-mid leading-relaxed">{bio}</p>
    </div>
  );
}
