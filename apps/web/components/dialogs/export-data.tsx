      ...
      <DialogContent className="max-w-2xl p-0 overflow-hidden border border-border-subtle rounded-sm shadow-lg">
        ...
        <div className="p-8 flex flex-col gap-8 bg-white overflow-y-auto max-h-[70vh] text-left">
          ...
          <div className="flex flex-col gap-4">
            <label className="font-ui text-[10px] text-amber-text uppercase tracking-widest font-bold">Select Period</label>
          ...
          {/* Section: Modules */}
          <div className="flex flex-col gap-4">
            <label className="font-ui text-[10px] text-amber-text uppercase tracking-widest font-bold">Select Modules</label>
          ...
          {/* Section: Format */}
          <div className="flex flex-col gap-4">
            <label className="font-ui text-[10px] text-amber-text uppercase tracking-widest font-bold">Export Format</label>
            <div className="flex gap-4">
              {[
                ...
                <label key={f.id} className="cursor-pointer flex-1">
                  ...
                  <div className="border border-border-subtle rounded-sm p-4 text-center peer-checked:border-amber peer-checked:bg-section-amber hover:bg-section-muted transition-colors flex flex-col items-center gap-2">
                    <Icon name={f.icon} className="text-mid ${format === f.id ? 'text-amber' : ''}" />
                    <span className={`font-ui text-xs ${format === f.id ? 'font-bold' : ''}`}>{f.name}</span>
                  </div>
                </label>
              ))}
            </div>
