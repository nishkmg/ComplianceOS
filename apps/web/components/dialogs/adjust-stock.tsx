      ...
      <DialogContent className="max-w-lg p-0 overflow-hidden border border-border-subtle shadow-2xl rounded-sm flex flex-col text-left">
        <DialogHeader className="px-8 pt-8 pb-6 border-b-[0.5px] border-border-subtle flex flex-row items-start justify-between">
          <div className="text-left">
            <DialogTitle className="font-display-lg text-lg font-bold text-on-surface">Adjust Stock</DialogTitle>
            <p className="font-ui-sm text-sm text-text-mid">Record inventory corrections or shrinkage.</p>
          </div>
        </DialogHeader>

        <div className="p-8 flex flex-col gap-8">
          <div className="bg-stone-50 border-[0.5px] border-border-subtle p-6 flex flex-col gap-4">
            <div className="flex justify-between items-start">
              <div className="text-left">
                <div className="font-ui-sm font-bold text-on-surface">{product.name}</div>
                <div className="font-mono text-[11px] text-text-light mt-0.5 uppercase tracking-wider">SKU: {product.sku}</div>
              </div>
              <div className="text-right">
                <label className="block font-ui-xs text-[10px] text-text-mid uppercase tracking-widest font-bold mb-1">Current Stock</label>
                <div className="font-mono text-lg text-on-surface font-bold">{product.available?.toLocaleString()}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6 text-left">
            <div className="flex gap-6">
              ...
              <div className="flex-1">
                <label className="block font-ui-xs text-[10px] text-text-mid uppercase tracking-widest font-bold mb-2">Warehouse</label>
                <div className="relative h-12">
                  <select 
                    className="w-full h-full bg-white border border-stone-300 rounded-sm px-4 py-2 font-ui-sm text-sm text-on-surface appearance-none focus:border-primary outline-none"
            ...
            <div>
              <label className="block font-ui-xs text-[10px] text-text-mid uppercase tracking-widest font-bold mb-2">Reason Code</label>
              <div className="relative h-12">
                <select 
                  className="w-full h-full bg-white border border-stone-300 rounded-sm px-4 py-2 font-ui-sm text-sm text-on-surface appearance-none focus:border-primary outline-none"
