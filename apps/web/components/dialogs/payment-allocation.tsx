      ...
      <DialogContent className="max-w-3xl p-0 overflow-hidden border border-border-subtle shadow-lg rounded-sm flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-8 text-left">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-ui text-md font-bold text-dark">Outstanding Invoices</h3>
            <div className="flex items-center gap-2">
              <Icon name="filter_list" className="text-light text-sm" />
              <span className="font-ui text-xs text-mid uppercase tracking-widest font-bold">Oldest First</span>
            </div>
          </div>
            ...
            {invoices.map((inv) => (
              <div key={inv.id} className="grid grid-cols-12 gap-4 py-4 border-b border-border-subtle items-center hover:bg-section-muted transition-colors group">
                <div className="col-span-1 flex items-center justify-center">
                  <input 
                    type="checkbox" 
                    className="rounded-sm border-border accent-primary w-4 h-4 cursor-pointer"
        ...
        <DialogFooter className="px-8 py-6 border-t border-border-subtle bg-section-muted flex flex-row items-center justify-between">
          ...
          <div className="flex gap-4">
            <button onClick={onClose} className="px-6 py-2 border border-border-subtle text-dark font-ui text-xs font-bold uppercase tracking-widest hover:bg-surface-muted transition-colors cursor-pointer bg-transparent">
              Cancel
            </button>
            <button 
