  ...
  return (
    <div className="space-y-6 text-left">
      <header className="flex justify-between items-center px-8 h-16 border-b border-border-subtle bg-white/80 -mx-8 -mt-8 mb-8">
        <h1 className="font-display-lg text-lg font-bold text-dark">Inventory Expiry Report</h1>
        <div className="flex items-center gap-4">
          <select className="border border-border-subtle rounded-sm py-1.5 px-3 text-xs bg-section-muted"><option>30 Days</option><option>60 Days</option></select>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse whitespace-nowrap">
          ...
          <tbody className="divide-y divide-stone-50 font-mono text-sm">
            {products.map((p) => (
              <tr key={p.sku} className="hover:bg-section-muted">
                <td className="px-6 py-4 text-primary font-medium">{p.sku}</td>
                <td className="px-6 py-4 font-ui-sm font-bold text-dark">{p.name}</td>
                ...
                <td className="px-6 py-4">
                  <span className={`inline-block px-2 py-0.5 text-[9px] font-bold uppercase rounded-sm border ${p.status === 'critical' ? 'bg-danger-bg text-danger border-red-200' : 'bg-section-amber text-amber-text border-amber-200'}`}>
