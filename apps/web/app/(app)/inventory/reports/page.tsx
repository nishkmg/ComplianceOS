    ...
    <div className="space-y-8 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border-subtle pb-6 mt-4">
        <div className="text-left">
          <h1 className="font-display-xl text-3xl text-dark mb-2 font-bold">Inventory Valuation</h1>
          <p className="font-ui-sm text-sm text-mid max-w-2xl leading-relaxed">Comprehensive breakdown of current stock levels, calculated asset values, and recent movement metrics across all registered warehouses.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 border border-border-subtle text-dark px-5 py-2.5 font-ui-sm text-xs font-bold uppercase tracking-widest hover:bg-section-muted transition-colors cursor-pointer bg-transparent rounded-sm shadow-sm">
      ...
      {/* Report Controls */}
      <div className="bg-white border border-border-subtle p-6 shadow-sm border-t-2 border-t-primary-container flex flex-col lg:flex-row gap-6 items-end">
        <div className="w-full lg:w-1/3 text-left">
          ...
          <div className="relative">
            <select 
              className="w-full bg-section-muted border border-border-subtle rounded-sm px-4 py-3 font-ui-sm text-sm text-dark focus:border-primary outline-none appearance-none"
              value={reportType}
        ...
        <div className="w-full lg:w-1/4 text-left">
          <label className="block font-ui-xs text-[10px] uppercase tracking-widest text-mid mb-2 font-bold">As of Date</label>
          <input className="w-full bg-section-muted border border-border-subtle rounded-sm px-4 py-3 font-mono text-sm text-dark focus:border-primary outline-none" type="date" defaultValue="2024-03-31" />
        </div>
        <button className="lg:ml-auto px-8 py-3 bg-section-dark text-white font-ui-sm text-xs font-bold uppercase tracking-widest hover:bg-black transition-colors rounded-sm border-none cursor-pointer shadow-sm">
          Run Analysis
        </button>
      </div>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-section-muted border-b-[0.5px] border-border-subtle text-light font-ui-xs text-[10px] uppercase tracking-widest">
            ...
            <tbody className="divide-y-[0.5px] divide-border-subtle font-mono text-[13px]">
              {reportData.map((row) => (
                <tr key={row.sku} className="hover:bg-section-muted/30 transition-colors">
                  ...
                  <td className="py-4 px-6 font-ui-sm font-bold text-dark">{row.name}</td>
                  <td className="py-4 px-6 font-ui-sm text-mid">{row.category}</td>
            ...
            <tfoot>
              <tr className="bg-section-dark text-white font-bold border-t-2 border-primary-container">
