            ...
            <tbody className="divide-y divide-border-subtle">
              {products.map(p => (
                <tr key={p.id} className="hover:bg-surface-muted transition-colors">
                  ...
                  <td className="py-3 px-6 font-mono text-[12px] text-mid">{p.hsnCode}</td>
                  <td className="py-3 px-6 text-right font-mono text-[13px] tabular-nums">{p.purchaseRate ? `₹${Number(p.purchaseRate).toLocaleString("en-IN")}` : "—"}</td>
                  <td className="py-3 px-6 text-right font-mono text-[13px] tabular-nums">{p.salesRate ? `₹${Number(p.salesRate).toLocaleString("en-IN")}` : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
