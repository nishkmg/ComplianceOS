    ...
    <div className="space-y-8 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
        <div>
          ...
          <h1 className="font-display-xl text-display-xl text-dark leading-none">Inventory Overview</h1>
          <p className="font-ui-sm text-ui-sm text-mid mt-3 max-w-lg">Strategic assessment of working capital locked in commodities and finished goods across all entities.</p>
        </div>
        <div className="flex gap-3">
          <Link href="/inventory/products/new" className="px-5 py-3 border border-border-subtle text-dark font-ui-sm text-sm rounded-sm hover:bg-section-muted transition-colors no-underline">
            Manage Products
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {kpiTiles.map((tile) => (
          <div key={tile.label} className={`bg-white border border-border-subtle border-t-2 p-6 shadow-sm hover:shadow-md transition-all ${
            ...
            <div className="flex justify-between items-start mb-4">
              <span className="font-ui-xs text-[10px] text-mid uppercase tracking-widest font-bold">{tile.label}</span>
            ...
            <div className={`font-mono text-2xl font-bold mb-2 ${tile.variant === 'amber' ? 'text-primary-container' : 'text-dark'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white border border-border-subtle p-8 shadow-sm h-[400px] flex flex-col">
            <h3 className="font-ui-lg text-lg font-bold text-dark mb-6">Stock Valuation Distribution</h3>
            <div className="flex-1 bg-section-muted border border-dashed border-border-subtle flex items-center justify-center relative overflow-hidden">
               <div className="text-center">
                 ...
                 <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest">Weighted Value by Category</p>
               </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-border-subtle shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b-[0.5px] border-border-subtle flex justify-between items-center bg-section-muted">
              <h3 className="font-ui-md font-bold text-dark">Critical Reorder List</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-section-muted/50 border-b-[0.5px] border-border-subtle">
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">SKU / Item</th>
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Balance</th>
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Warehouse</th>
              ...
              <tbody className="divide-y-[0.5px] divide-border-subtle font-mono text-[13px]">
                {lowStock.map((item) => (
                  <tr key={item.sku} className="hover:bg-section-muted transition-colors">
                    <td className="py-4 px-6 text-left">
                      <div className="font-ui-sm font-bold text-dark">{item.name}</div>
                    ...
                    <td className="py-4 px-6 font-ui-sm text-mid">{item.warehouse}</td>
                    <td className="py-4 px-6">
                      <span className={`inline-block px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider border rounded-sm ${
                        item.status === 'critical' ? 'bg-danger-bg text-danger border-red-200' : 'bg-section-amber text-amber-text border-amber-200'
                      }`}>
                        {item.status}
                      </span>
                    </td>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-section-dark p-8 border-l-4 border-l-primary-container text-left shadow-sm">
          ...
          <div className="bg-section-amber border border-amber/30 p-8 shadow-sm text-left relative overflow-hidden group">
            <div className="relative z-10">
              <Icon name="analytics" className="text-primary-container mb-4" />
              <h4 className="font-ui-lg text-lg font-bold text-dark mb-2">Dead Stock Analysis</h4>
              <p className="font-ui-sm text-sm text-mid leading-relaxed">System has identified items worth ₹ 2.4L that haven't moved in 180 days. Consider liquidation or write-down.</p>
            </div>
