      ...
      {stock.length === 0 ? <EmptyState icon="inventory_2" title="No stock" description="Stock layers will appear here once inventory movements are recorded." /> : (
        <div className="bg-surface border border-border rounded-md shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse"><thead><tr className="bg-surface-muted border-b border-border">
            <th className="py-3 px-6 font-ui text-ui-2xs text-light uppercase tracking-widest">Product</th>
            <th className="py-3 px-6 font-ui text-ui-2xs text-light uppercase tracking-widest text-right">Qty</th>
            <th className="py-3 px-6 font-ui text-ui-2xs text-light uppercase tracking-widest text-right">Remaining</th>
            <th className="py-3 px-6 font-ui text-ui-2xs text-light uppercase tracking-widest text-right">Unit Cost</th>
            <th className="py-3 px-6 font-ui text-ui-2xs text-light uppercase tracking-widest text-right">Total Value</th>
            <th className="py-3 px-6 font-ui text-ui-2xs text-light uppercase tracking-widest">Receipt Date</th>
            ...
            {stock.map(s => (
              <tr key={s.id} className="hover:bg-surface-muted transition-colors">
                ...
                <td className="py-3 px-6 text-right font-mono text-ui-sm tabular-nums">{formatIndianNumber(Number(s.totalValue || 0), { currency: true })}</td>
                <td className="py-3 px-6 font-mono text-ui-xs text-mid">{s.receiptDate ? new Date(s.receiptDate).toLocaleDateString("en-IN") : "—"}</td>
              </tr>
            ))}
          </tbody></table>
        </div>
