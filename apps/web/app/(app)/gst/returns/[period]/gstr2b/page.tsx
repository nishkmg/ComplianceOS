...
export default function Gstr2bPage() {
  ...
  const returns = api.gstReturns.list.useQuery(
    { periodMonth, periodYear, returnType: "gstr2b" },
    ...
    <div className="max-w-page mx-auto space-y-8 pb-40">
      ...
      {returns.isLoading ? (
        ...
        <EmptyState
          ...
          description="GSTR-2B is auto-drafted from supplier filings; generate it to import the ITC statement."
          action={{ label: "Generate GSTR-2B", onClick: () => generate.mutate({ periodMonth, periodYear }) }}
        />
      ) : (
          ...
          <div className="flex items-center justify-between bg-surface border border-border rounded-md px-6 py-4 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              {(ret.status === "generated" || ret.status === "filed") && (
                <>
                  <Link href={`/api/gst/returns/${ret.id}/csv?type=gstr2b`} className={buttonVariants({ variant: "outline", size: "sm" })}>
                    <Icon name="download" size={14} /> CSV
                  </Link>
                  <Link href={`/api/gst/returns/${ret.id}/pdf?type=gstr2b`} target="_blank" className={buttonVariants({ variant: "outline", size: "sm" })}>
