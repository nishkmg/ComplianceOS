...
export default function Gstr3bPage() {
  ...
  const returns = api.gstReturns.list.useQuery(
    { periodMonth, periodYear, returnType: "gstr3b" },
  ...
  return (
    <div className="max-w-page mx-auto space-y-8 pb-40">
      ...
      {returns.isLoading ? (
        ...
        <EmptyState
          icon="description"
          title="No GSTR-3B prepared for this period"
          description="GSTR-3B is computed from outward supplies and ITC; generate it to prepare the summary."
          action={{ label: "Generate GSTR-3B", onClick: () => generate.mutate({ periodMonth, periodYear }) }}
        />
      ) : (
          ...
          <div className="flex items-center justify-between bg-surface border border-border rounded-md px-6 py-4 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              {(ret.status === "generated" || ret.status === "filed") && (
                <>
                  <Link href={`/api/gst/returns/${ret.id}/csv?type=gstr3b`} className={buttonVariants({ variant: "outline", size: "sm" })}>
                    <Icon name="download" size={14} /> CSV
                  </Link>
                  <Link href={`/api/gst/returns/${ret.id}/pdf?type=gstr3b`} target="_blank" className={buttonVariants({ variant: "outline", size: "sm" })}>
