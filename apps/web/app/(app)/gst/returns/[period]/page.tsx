      ...
      <div className="card p-5">
        ...
        {currentReturn ? (
          <div className="space-y-2 font-ui text-[13px]">
            <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Return Number</span><span className="font-mono text-dark">{currentReturn.returnNumber}</span></div>
            <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Return Type</span><span className="font-mono text-dark uppercase">{currentReturn.returnType}</span></div>
            <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Status</span><Badge variant={statusConfig[currentReturn.status]?.variant as BadgeVariant || "gray"}>{statusConfig[currentReturn.status]?.label || currentReturn.status}</Badge></div>
            <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Due Date</span><span className="font-mono text-dark">{currentReturn.dueDate}</span></div>
            {currentReturn.filingDate && <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Filed Date</span><span className="font-mono text-dark">{currentReturn.filingDate}</span></div>}
            <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Total Tax Payable</span><span className="font-mono text-dark">{formatIndianNumber(Number(currentReturn.totalTaxPayable))}</span></div>
            <div className="flex justify-between py-2 border-b border-border-subtle"><span className="text-light">Total ITC</span><span className="font-mono text-dark">{formatIndianNumber(Number(currentReturn.totalEligibleItc))}</span></div>
