    ...
    <div className="space-y-8 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="font-display-xl text-display-xl text-dark leading-tight">GST Returns</h1>
          <p className="font-ui-sm text-mid mt-2 max-w-2xl">Centralized management for your GST compliance cycle. Generate, reconcile and file returns directly with the GSTN portal.</p>
        </div>
        <div className="flex gap-3">
          <select className="border border-border-subtle px-4 py-2 text-ui-sm outline-none bg-white" value={period} onChange={(e) => setPeriod(e.target.value)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {returnTypes.map((t) => (
          <div key={t.id} className="bg-white border border-border-subtle p-6 shadow-sm relative overflow-hidden group hover:border-primary transition-colors">
            ...
            <div className="flex justify-between items-start mb-4">
              <span className="font-ui-xs text-light uppercase tracking-widest">{t.id}</span>
              <Icon name="article" className="text-primary-container" />
            </div>
            <h3 className="font-display-lg text-display-lg text-dark mb-1">{t.name}</h3>
            <p className="font-ui-sm text-mid mb-6">{t.desc}</p>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b-[0.5px] border-border-subtle bg-sidebar">
          <h3 className="font-ui-md font-bold text-dark uppercase tracking-wider text-[11px] text-light">Return Filing History</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-section-muted border-b-[0.5px] border-border-subtle">
                <th className="py-3 px-6 font-ui-xs text-light uppercase tracking-widest">Period</th>
                <th className="py-3 px-6 font-ui-xs text-light uppercase tracking-widest">Return Type</th>
                <th className="py-3 px-6 font-ui-xs text-light uppercase tracking-widest text-right">Amount (₹)</th>
                <th className="py-3 px-6 font-ui-xs text-light uppercase tracking-widest text-right">Status</th>
                <th className="py-3 px-6 font-ui-xs text-light uppercase tracking-widest text-right">Date Filed</th>
              </tr>
            </thead>
            <tbody className="divide-y-[0.5px] divide-border-subtle">
              {mockReturns.map((r) => (
                <tr key={r.id} className="hover:bg-section-muted/30 transition-colors">
                  <td className="py-4 px-6 font-mono-md text-dark">{r.period}</td>
                  <td className="py-4 px-6 font-ui-sm font-bold uppercase text-dark">{r.type}</td>
                  ...
                  <td className="py-4 px-6 text-right">
                    <span className={`inline-block px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider border rounded-sm ${
                      ...
                      r.status === 'generated' ? 'bg-section-amber text-amber-text border-amber-200' :
                      'bg-blue-50 text-blue-700 border-blue-200'
                    }`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="py-4 px-6 font-mono-md text-right text-mid">{r.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
