  ...
  return (
    <div className="space-y-0 text-left">
      ...
      <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b-[0.5px] border-border-subtle pb-8 mt-4">
        <div>
          <h1 className="font-display-xl text-display-xl text-dark mb-2 font-bold">Liability Ledger</h1>
          <p className="font-ui-sm text-sm text-mid flex items-center gap-2">
            <Icon name="calendar_today" className="text-[16px]" />
            Current Period: April 2023 - March 2024
          </p>
        </div>
        <div className="flex gap-4">
          <button className="px-5 py-2 border border-border-subtle text-dark font-ui-sm text-xs font-bold uppercase tracking-widest hover:bg-section-muted transition-colors cursor-pointer bg-transparent rounded-sm shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-white border border-border-subtle p-6 flex flex-col justify-between shadow-sm border-t-2 border-t-primary-container text-left">
          ...
          <div className="flex items-end justify-between mt-4">
            ...
            <Icon name="trending_up" className="text-lighter group-hover:text-primary-container transition-colors" />
          </div>
        </div>
        <div className="bg-white border border-border-subtle p-6 flex flex-col justify-between shadow-sm text-left">
          <span className="font-ui-xs text-[10px] text-mid uppercase tracking-widest font-bold">CGST Output</span>
          <span className="font-mono text-xl font-bold text-dark mt-4">₹ 4,15,200.00</span>
        </div>
        <div className="bg-white border border-border-subtle p-6 flex flex-col justify-between shadow-sm text-left">
          <span className="font-ui-xs text-[10px] text-mid uppercase tracking-widest font-bold">SGST Output</span>
          <span className="font-mono text-xl font-bold text-dark mt-4">₹ 4,15,200.00</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y-[0.5px] divide-border-subtle font-mono text-[13px]">
              {[
                ...
                <tr key={i} className="hover:bg-section-muted/30 transition-colors">
                  <td className="py-5 px-6 font-ui-sm font-bold text-dark">{r.period}</td>
            ...
            <tfoot>
              <tr className="bg-section-dark text-white font-bold">
