      ...
      <header className="bg-white border-b-[0.5px] border-border-subtle px-8 py-10 -mx-8 -mt-8 mb-8 sticky top-0 z-20 backdrop-blur-sm bg-white/50">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="font-display-xl text-display-xl text-dark mb-1 font-bold">Cash Ledger</h1>
            <p className="font-ui-md text-sm text-mid">Electronic tracking of cash deposited to the GST portal.</p>
          </div>
          <div className="flex items-center gap-3 bg-section-muted p-1 rounded-sm border border-border-subtle">
            <button className="px-4 py-1.5 font-ui-sm text-xs text-mid hover:text-dark transition-colors cursor-pointer border-none bg-transparent font-bold">Oct 2024</button>
      ...
      <div className="max-w-6xl mx-auto space-y-12 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {Object.entries(mockBalances).map(([key, val]) => (
            <div key={key} className="bg-white border border-border-subtle border-t-2 border-t-primary-container p-6 shadow-sm hover:shadow-md transition-shadow text-left">
              <div className="font-ui-xs text-[10px] text-light mb-3 uppercase tracking-widest font-bold">{key} Balance</div>
        ...
        <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-4 bg-section-muted border-b border-border-subtle flex justify-between items-center">
              <h3 className="font-ui-md font-bold text-dark uppercase tracking-wider text-[11px] text-light">Electronic Cash Statement</h3>
              ...
              <tbody className="divide-y divide-stone-50 font-mono text-[13px]">
                {mockTransactions.map((t) => (
                  <tr key={t.id} className="hover:bg-section-muted/30 transition-colors">
                    ...
                    <td className="py-5 px-6 font-ui-sm font-bold text-dark">{t.desc}</td>
