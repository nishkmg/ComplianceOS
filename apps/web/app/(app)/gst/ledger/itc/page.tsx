...
export default function ITCLedgerPage() {
  ...
  const { data: transactions } = api.gstLedger.ledgerTransactions.useQuery({ type: "itc", periodMonth, periodYear });
  const filteredTransactions = selectedTaxType === "all" ? transactions : transactions?.filter(t => t.taxType === selectedTaxType);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="font-display text-[26px] font-normal text-dark mt-1">ITC Ledger</h1>
        </div>
        <Link href="/gst/payment" className="filter-tab active">Utilize ITC</Link>
      </div>
      ...
      <div className="flex gap-4 items-center flex-wrap">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select value={selectedTaxType} onChange={(e) => setSelectedTaxType(e.target.value)} className="input-field font-ui">
            <option value="all">All Tax Types</option>
            {taxTypes.map((t) => (<option key={t} value={t}>{t.toUpperCase()}</option>))}
          </select>
        </div>
      </div>
        ...
        {taxTypes.map((type) => {
          ...
          return (
            <div key={type} className="card p-4">
              <p className="font-ui text-[10px] uppercase tracking-wide text-light mb-3">{type.toUpperCase()}</p>
              <div className="space-y-2 font-ui text-[12px]">
                <div className="flex justify-between"><span className="text-light">Opening:</span><span className="font-mono text-dark">{formatIndianNumber(data.openingBalance)}</span></div>
                <div className="flex justify-between"><span className="text-light">ITC Added:</span><span className="font-mono text-success">{formatIndianNumber(data.itcAvailable)}</span></div>
                <div className="flex justify-between"><span className="text-light">Reversed:</span><span className="font-mono text-danger">-{formatIndianNumber(data.itcReversed)}</span></div>
                <div className="flex justify-between"><span className="text-light">Utilized:</span><span className="font-mono text-amber">{formatIndianNumber(data.itcUtilized)}</span></div>
                <div className="border-t border-hairline pt-2 mt-2 flex justify-between font-medium"><span className="text-dark">Closing:</span><span className="font-mono text-dark">{formatIndianNumber(data.closingBalance)}</span></div>
              </div>
            </div>
          );
        })}
      ...
      <div className="card overflow-hidden">
        <div className="px-4 py-3 border-b border-hairline">
          <h2 className="font-display text-[16px] font-normal text-dark">ITC Transactions</h2>
        </div>
            ...
            {filteredTransactions && filteredTransactions.length > 0 ? (
              filteredTransactions.map((t) => (
                <tr key={t.id} className="border-b border-hairline">
                  ...
                  <td className="font-mono text-[13px] text-light px-4 py-3">{t.referenceNumber || "-"}</td>
                </tr>
              ))
            ) : (
