  ...
  return (
    <div className="space-y-8 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b-[0.5px] border-border-subtle pb-8 mt-4">
        <div>
          <h2 className="font-display-xl text-display-xl text-dark mb-2 font-bold">Electronic Credit Ledger</h2>
          <p className="font-ui-sm text-sm text-mid">Statement of Input Tax Credit for <span className="font-mono text-dark font-bold text-[13px]">GSTIN: 27AACPB1234F1Z5</span></p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex flex-col gap-1 text-left">
            <label className="text-[10px] uppercase text-mid font-bold tracking-widest">Tax Period</label>
            <div className="flex gap-2">
              <select className="bg-section-muted border border-border-subtle rounded-sm py-2 px-3 text-xs outline-none focus:border-primary" value={month} onChange={e => setMonth(Number(e.target.value))}>
                {months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
              </select>
              <select className="bg-section-muted border border-border-subtle rounded-sm py-2 px-3 text-xs outline-none focus:border-primary" value={year} onChange={e => setYear(Number(e.target.value))}>
                <option>2024</option>
                <option>2023</option>
              </select>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-border-subtle border-t-2 border-t-primary-container p-6 shadow-sm hover:shadow-md transition-shadow text-left">
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-section-muted border-b border-border-subtle flex justify-between items-center">
            <h3 className="font-ui-md font-bold text-dark uppercase tracking-wider text-[11px] text-light">Ledger Transaction History</h3>
              ...
              {mockTransactions.map((t) => (
                <tr key={t.id} className="hover:bg-section-muted/30 transition-colors">
                  ...
                  <td className="py-5 px-6 text-left">
                    <div className="font-ui-sm font-bold text-dark text-sm">{t.type}</div>
                    <div className="text-[11px] text-light mt-0.5">{t.ref}</div>
                  </td>
                  <td className="py-5 px-6 font-ui-xs text-[10px] uppercase font-bold text-mid">Credit</td>
                  <td className="py-5 px-6 font-ui-sm font-bold text-dark">{t.taxType}</td>
