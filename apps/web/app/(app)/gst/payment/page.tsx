    ...
    <div className="space-y-0 text-left">
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b-[0.5px] border-border-subtle px-8 py-6 -mx-8 -mt-8 mb-8 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button className="px-6 py-2.5 border border-border-subtle text-dark font-ui-sm text-xs font-bold uppercase tracking-widest hover:bg-section-muted transition-colors cursor-pointer bg-transparent rounded-sm shadow-sm">Save Draft</button>
      ...
      <div className="space-y-16 pb-16 max-w-[1200px] mx-auto">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end">
           <div className="flex flex-col gap-2 text-left">
             <label className="font-ui-xs text-xs text-light uppercase tracking-widest font-bold">Return Period</label>
             <div className="relative">
               <select className="w-full bg-white border border-border-subtle rounded-sm px-4 py-3 font-ui-sm text-sm text-dark focus:border-primary outline-none appearance-none" value={period} onChange={e => setPeriod(e.target.value)}>
        ...
        <section>
          <div className="flex justify-between items-end mb-6 border-b-[0.5px] border-border-subtle pb-4">
            <h3 className="font-ui-lg text-lg font-bold text-dark">1. Gross Tax Liability</h3>
            <span className="font-ui-xs text-[10px] text-light uppercase tracking-widest font-bold">Derived from Outward Supplies (GSTR-1)</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {Object.entries(grossLiability).filter(([k]) => k !== 'total').map(([key, val]) => (
              <div key={key} className="bg-white border border-border-subtle p-6 border-t-2 border-t-transparent hover:border-t-primary-container transition-all shadow-sm text-left">
                <span className="font-ui-xs text-[10px] text-light uppercase font-bold tracking-widest">{key === 'igst' ? 'Integrated Tax (IGST)' : key === 'cgst' ? 'Central Tax (CGST)' : key === 'sgst' ? 'State Tax (SGST)' : 'Cess'}</span>
                <div className="flex items-baseline gap-1 mt-4">
                  <span className="font-ui-sm text-xs text-light">₹</span>
                  <span className="font-mono text-2xl font-bold text-dark">{formatIndianNumber(val)}</span>
                </div>
              </div>
            ))}
          </div>
          <div className="bg-section-dark text-white p-6 mt-6 flex justify-between items-center shadow-sm border border-stone-950">
            <span className="font-ui-sm text-xs font-bold uppercase tracking-widest text-light">Total Gross Liability</span>
        ...
        <section>
          <div className="flex justify-between items-end mb-6 border-b-[0.5px] border-border-subtle pb-4">
            <h3 className="font-ui-lg text-lg font-bold text-dark">2. ITC Set-off Computation</h3>
            <span className="font-ui-xs text-[10px] text-light uppercase font-bold tracking-widest">Cross-utilization as per GST rules (IGST first)</span>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-stone-50 font-mono text-sm">
                {['igst', 'cgst', 'sgst'].map((type, i) => (
                  <tr key={type} className="hover:bg-section-muted/50 transition-colors">
        ...
        {/* 4. Net Payable */}
        <section className="bg-white border border-border-subtle p-8 shadow-sm border-t-2 border-t-red-600">
          <h3 className="font-ui-lg text-lg font-bold text-dark mb-8">3. Net Cash Payable</h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {Object.entries(netPayable).filter(([k]) => k !== 'total').map(([key, val]) => (
              <div key={key} className="bg-section-muted border border-border-subtle p-6 text-left">
                <span className="font-ui-xs text-[10px] text-light uppercase font-bold tracking-widest">{key.toUpperCase()}</span>
          ...
          <div className="bg-section-dark text-white p-8 mt-6 flex justify-between items-center shadow-xl border border-stone-950">
            <span className="font-ui-sm text-xs font-bold uppercase tracking-widest text-amber-text">Due by 20th November 2024</span>
            <div className="text-right">
              <p className="font-ui-xs text-[10px] text-light uppercase font-bold tracking-widest mb-1">Total Amount Payable</p>
