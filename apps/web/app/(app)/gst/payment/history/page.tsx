      ...
      {payments.length === 0 ? <EmptyState icon="payments" title="No payments yet" description="GST challan payments will appear here." /> : (
        <div className="bg-surface border border-border rounded-md shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse"><thead><tr className="bg-surface-muted border-b border-border">
            <th className="py-3 px-6 font-ui text-[10px] text-light uppercase tracking-widest">Challan</th>
            ...
            <th className="py-3 px-6 font-ui text-[10px] text-light uppercase tracking-widest">Bank</th>
            ...
            {payments.map(p => (
              <tr key={p.id} className="hover:bg-surface-muted transition-colors">
                ...
                <td className="py-3 px-6 text-right font-mono text-[13px] tabular-nums">₹{Number(p.amount || 0).toLocaleString("en-IN")}</td>
              </tr>
            ))}
          </tbody></table>
        </div>
