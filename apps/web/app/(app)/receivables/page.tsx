    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display-lg text-display-lg text-dark leading-tight">Receivables Summary</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-border-subtle p-6 shadow-sm rounded-sm border-t-4 border-t-primary-container">
          <p className="font-ui-xs text-[10px] text-mid uppercase tracking-widest mb-3 font-bold">Total Outstanding</p>
        ...
        <div className="bg-white border border-border-subtle p-6 shadow-sm rounded-sm border-t-4 border-t-danger">
          <p className="font-ui-xs text-[10px] text-mid uppercase tracking-widest mb-3 font-bold">Overdue (&gt;30 Days)</p>
          <p className="font-mono text-2xl font-bold text-danger tabular-nums">
            {formatIndianNumber(totalOverdue, { currency: true })}
          </p>
        </div>
        <div className="bg-white border border-border-subtle p-6 shadow-sm rounded-sm border-t-4 border-t-dark">
          <p className="font-ui-xs text-[10px] text-mid uppercase tracking-widest mb-3 font-bold">Avg. Collection Period</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7">
          <div className="bg-white border border-border-subtle p-6 shadow-sm rounded-sm">
            <h3 className="font-ui-sm text-[13px] font-bold text-dark mb-6 uppercase tracking-widest">Aging Breakdown</h3>
            <div className="space-y-5">
              {agingBuckets.map(bucket => (
                <div key={bucket.label}>
                  <div className="flex justify-between items-end mb-2">
                    <span className="font-ui-sm text-[13px] font-medium text-dark">{bucket.label}</span>
                    <div className="text-right">
                      ...
                      <span className="font-ui-xs text-[10px] text-light">{bucket.percentage}%</span>
                    </div>
                  </div>
                  <div className="w-full bg-lighter/60 h-1.5 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-1000 rounded-full ${
                        bucket.label.includes(">") ? "bg-danger" : "bg-primary-container"
                      }`}
        ...
        <div className="lg:col-span-5">
          <div className="bg-white border border-border-subtle shadow-sm rounded-sm overflow-hidden">
            <div className="px-6 py-4 bg-stone-50 border-b border-border-subtle flex justify-between items-center">
              <h3 className="font-ui-sm text-[13px] font-bold text-dark uppercase tracking-widest">Top Debtors</h3>
              <Link href="/receivables/list" className="text-[10px] text-primary-container font-bold uppercase tracking-widest hover:underline no-underline">
                View All
              </Link>
            </div>
            <div className="divide-y divide-border-subtle">
              {topDebtors.map(d => (
                <div key={d.name} className="px-6 py-5 flex justify-between items-center hover:bg-stone-50/50 transition-colors">
                  <div>
                    <p className="font-ui-sm text-[13px] font-semibold text-dark">{d.name}</p>
                    <span className={`inline-block px-2 py-0.5 mt-1.5 text-[9px] uppercase font-bold tracking-widest border rounded-sm ${
                      d.status === "overdue"
                  ...
                  <div className="text-right">
                    ...
                    <Link
                      href={`/receivables/1`}
                      className="text-[10px] text-light hover:text-primary-container transition-colors no-underline font-bold uppercase tracking-widest"
                    >
                      Statement →
                    </Link>
                  </div>
