...
import { ReceiptScanResults } from "@/components/ocr/receipt-scan-results";
...
async function uploadScan(fileUrl: string, fileName: string, fileSize: number): Promise<{ scanId: string }> {
  const response = await fetch("/api/trpc/ocrScan.upload", {
    ...
    body: JSON.stringify({
      input: { fileUrl, fileName, fileSize, scanType: "receipt" },
    }),
  });
...
export default function ScanReceiptPage() {
  ...
  const handleExpenseCreated = useCallback((entryId: string) => {
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Upload a vendor receipt or bill to automatically extract expense details and create a draft journal entry.
        </p>
      </div>
