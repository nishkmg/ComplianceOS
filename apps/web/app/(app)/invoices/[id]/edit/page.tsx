...
export default function EditInvoicePage() {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>(mockInvoice.lineItems.map((item) => ({ ...item })));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-[26px] font-normal text-dark">Edit Invoice</h1>
          <p className="font-ui text-[12px] text-light mt-1">{mockInvoice.invoiceNumber}</p>
        </div>
        <div className="flex gap-2">
          <button type="button" onClick={() => router.back()} className="filter-tab">
            Cancel (Esc)
          </button>
          <button type="submit" className="filter-tab">
            Save Changes (⌘S)
          </button>
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                <label className="font-ui text-[10px] uppercase tracking-wide text-light">Customer Name</label>
                <input
                  type="text"
                  value={customer.name}
