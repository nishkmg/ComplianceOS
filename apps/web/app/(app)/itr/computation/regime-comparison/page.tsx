    ...
    <div className="space-y-0 text-left">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h2 className="font-display-xl text-display-xl text-dark mb-2">Regime Comparison</h2>
          <p className="font-ui-sm text-sm text-mid max-w-2xl leading-relaxed">A detailed analysis of tax liability under the Old and New tax regimes based on current inputs.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="border border-stone-200 text-dark py-2 px-4 rounded-sm font-ui-sm text-xs font-bold uppercase tracking-widest hover:bg-section-muted transition-colors flex items-center gap-2 cursor-pointer bg-white shadow-sm">
      ...
      <div className="bg-success-bg border border-green-200 p-6 mb-12 flex items-start gap-4">
        ...
        <div>
          <h3 className="font-display-lg text-lg font-bold text-dark mb-1">New Regime Recommended</h3>
          <p className="font-ui-sm text-sm text-mid">Opting for the New Regime saves <span className="font-mono text-dark font-bold text-base">₹ 45,200</span> in total tax liability for the current assessment year.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
        {/* Old Regime Card */}
        <div className="bg-white border border-border-subtle shadow-sm flex flex-col">
          <div className="p-6 border-b-[0.5px] border-border-subtle bg-section-muted">
            <h3 className="font-display-lg text-lg font-bold text-mid uppercase tracking-widest text-xs">Old Tax Regime</h3>
          </div>
          <div className="flex-1 divide-y-[0.5px] divide-border-subtle font-mono text-[13px]">
            {comparisonData.map((row, i) => (
              <div key={i} className="flex justify-between items-center p-6 hover:bg-section-muted transition-colors">
                <span className="font-ui-sm text-mid text-xs uppercase tracking-wider">{row.label}</span>
                <span className="text-dark">₹ {formatIndianNumber(row.old)}</span>
              </div>
            ))}
          </div>
          <div className="p-8 bg-section-muted border-t-2 border-stone-800">
            <div className="flex justify-between items-center">
              <span className="font-ui-sm font-bold uppercase tracking-widest text-xs text-mid">Final Liability</span>
        ...
        {/* New Regime Card */}
        <div className="bg-white border border-primary-container shadow-lg flex flex-col relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-[2px] bg-primary-container"></div>
          <div className="p-6 border-b-[0.5px] border-primary-container/20 bg-section-amber">
            <div className="flex justify-between items-center">
          ...
          <div className="flex-1 divide-y-[0.5px] divide-border-subtle font-mono text-[13px]">
            {comparisonData.map((row, i) => (
              <div key={i} className="flex justify-between items-center p-6 hover:bg-section-amber/50 transition-colors">
                <span className="font-ui-sm text-mid text-xs uppercase tracking-wider">{row.label}</span>
                <span className={row.new === 0 ? "text-light opacity-50" : "text-dark"}>₹ {formatIndianNumber(row.new)}</span>
              </div>
            ))}
          </div>
          <div className="p-8 bg-section-amber border-t-2 border-primary">
            <div className="flex justify-between items-center">
