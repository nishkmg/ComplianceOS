    ...
    <div className="space-y-0 text-left">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border-subtle pb-6">
        <div>
          ...
          <h2 className="font-display-xl text-display-xl text-dark mb-2">Advance Tax Tracking</h2>
          <p className="font-ui-sm text-sm text-mid max-w-2xl leading-relaxed">Ensure timely payment of advance tax instalments to avoid penal interest under Section 234B and 234C.</p>
        </div>
        <div className="flex gap-3">
          <button className="px-5 py-2 border border-border-subtle text-dark font-ui-sm text-xs rounded-sm hover:bg-section-muted transition-colors flex items-center gap-2 cursor-pointer bg-white shadow-sm uppercase font-bold tracking-widest">
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        {/* Liability Summary */}
        <section className="bg-section-dark text-white p-10 flex flex-col md:flex-row justify-between items-center gap-8 shadow-xl border border-stone-950">
          <div className="text-left flex-1">
            <h3 className="text-amber-text font-ui-lg text-lg font-bold mb-2">Projected Annual Liability</h3>
            <p className="text-light font-ui-sm text-sm leading-relaxed">Based on your current fiscal P&L data and estimated non-business income for FY 2024-25.</p>
          </div>
          <div className="text-right">
            <p className="text-light font-ui-xs text-[10px] uppercase tracking-[0.2em] mb-2">Estimated Net Tax</p>
        ...
        <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-4 bg-section-muted border-b border-border-subtle">
            <h3 className="font-ui-md font-bold text-dark uppercase tracking-wider text-[11px] text-light">Instalment Schedule</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-section-muted border-b-[0.5px] border-border-subtle">
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Instalment</th>
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Due Date</th>
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Target</th>
                  ...
                  <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Date Paid</th>
                </tr>
              </thead>
              <tbody className="divide-y-[0.5px] divide-border-subtle font-mono text-[13px]">
                {instalments.map((i) => (
                  <tr key={i.id} className="hover:bg-section-muted/30 transition-colors">
                    <td className="py-5 px-6 font-ui-sm font-bold text-dark">{i.label}</td>
                    ...
                    <td className="py-5 px-6">
                      <span className={`inline-block px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider border rounded-sm ${
                        i.status === 'paid' ? 'bg-success-bg text-success border-green-200' : 'bg-section-amber text-amber-text border-amber-200'
                      }`}>
                        {i.status}
                      </span>
                    </td>
