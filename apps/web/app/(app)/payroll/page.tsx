...
const columns: ColumnDef<PayrollRun>[] = [
  { key: "payroll_number", header: "Run #", width: "150px", render: (r) => <Link href={`/payroll/${r.id}`} className="font-mono text-ui-xs text-amber hover:underline no-underline">{r.payroll_number}</Link> },
  { key: "month", header: "Period", sortable: true, render: (r) => <span className="font-ui text-ui-sm text-dark">{r.month} {r.year}</span> },
  { key: "net_pay", header: "Net Pay", align: "right", render: (r) => <span className="font-mono text-ui-sm tabular-nums">₹{Number(r.net_pay || 0).toLocaleString("en-IN")}</span> },
