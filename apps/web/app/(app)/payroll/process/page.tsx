  ...
  return (
    <div className="space-y-0 text-left">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-border-subtle pb-6">
        <div>
          <h2 className="font-display-xl text-display-xl text-dark mb-2">Process Payroll</h2>
          <p className="font-ui-md text-sm text-mid">October 2023 · {mockEmployees.length} Employees</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="px-4 py-2 border border-border-subtle font-ui-sm text-sm rounded-sm hover:bg-section-muted transition-colors text-dark cursor-pointer bg-white">Save Draft</button>
      ...
      <div className="mb-8 border-b border-border-subtle pb-4 overflow-x-auto no-print">
        <div className="flex items-center gap-8 min-w-max">
          {[
            ...
            <div key={s.n} className={`flex items-center gap-2 ${step === s.n ? 'text-primary' : 'text-mid'}`}>
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold ${
                step === s.n ? 'bg-primary-container text-white' : 
                step > s.n ? 'bg-green-100 text-success' : 'bg-stone-100 text-light'
              }`}>
                {step > s.n ? '✓' : s.n}
              </span>
      ...
      {/* Warning Panel */}
      <div className="bg-section-amber border-l-4 border-amber-500 p-6 mb-8 flex gap-4 text-left">
        <Icon name="warning" className="text-amber-text" />
        <div>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-section-muted border-b border-border-subtle flex justify-between items-center">
          <h3 className="font-ui-md font-bold text-dark">Employee Earnings Summary</h3>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-section-muted border-b-[0.5px] border-border-subtle">
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Employee</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Gross Earnings</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">PF (12%)</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">ESI (0.75%)</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">TDS</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right font-bold">Net Payable</th>
              </tr>
            </thead>
              ...
              {mockEmployees.map((e) => (
                <tr key={e.id} className="hover:bg-section-muted/30 transition-colors">
                  <td className="py-4 px-6 text-left">
                    <div className="font-ui-sm font-bold text-dark text-sm">{e.name}</div>
                    <div className="text-[11px] text-light mt-0.5">{e.code}</div>
                  </td>
                  <td className="py-4 px-6 text-right text-mid">{formatIndianNumber(e.gross)}</td>
            ...
            <tfoot>
              <tr className="bg-section-muted font-bold border-t-2 border-border-subtle">
                ...
                <td colSpan={3} className="py-4 px-6 text-right font-ui-xs uppercase tracking-widest text-[10px] text-light">Net Liability</td>
