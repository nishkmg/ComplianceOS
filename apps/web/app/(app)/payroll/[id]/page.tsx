...
// @ts-ignore
  ...
  return (
    <div className="space-y-6 text-left">
      {/* Breadcrumbs */}
      <div className="flex items-center gap-2 font-ui-xs text-[10px] text-light uppercase tracking-widest mb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b-[0.5px] border-border-subtle pb-8">
        <div>
          <h1 className="font-display-xl text-display-xl text-dark mb-4 font-bold">Payroll Detail</h1>
          <div className="flex items-center gap-6 font-ui-sm text-sm text-mid">
        ...
        <div className="flex gap-4">
          <button className="px-6 py-2 border border-border-subtle text-dark font-ui-sm text-xs font-bold uppercase tracking-widest hover:bg-section-muted transition-colors cursor-pointer bg-transparent rounded-sm shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="bg-white border border-border-subtle p-8 border-t-2 border-t-stone-300 shadow-sm">
          <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-4 font-bold">Gross Earnings</p>
          <p className="font-mono text-3xl font-bold text-dark">₹ {formatIndianNumber(totalGross)}</p>
        </div>
        <div className="bg-white border border-border-subtle p-8 border-t-2 border-t-red-600 shadow-sm">
          <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-4 font-bold">Total Deductions</p>
        ...
        <div className="bg-white border border-border-subtle p-8 border-t-2 border-t-primary-container shadow-sm">
          <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-4 font-bold">Net Take-Home</p>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-section-muted border-b border-border-subtle flex justify-between items-center">
            <h3 className="font-ui-md font-bold text-dark uppercase tracking-wider text-[11px] text-light">Employee Payroll Breakdown</h3>
            <span className={`px-3 py-1 text-[10px] font-bold uppercase tracking-widest border rounded-sm ${run.status === 'finalized' ? 'bg-success-bg text-success border-green-200' : 'bg-section-amber text-amber-text border-amber-200'}`}>
              {run.status}
            </span>
        </div>
              ...
              {(lines as any[]).map((line: any) => (
                <tr key={line.id} className="hover:bg-section-muted/30 transition-colors">
                  <td className="py-5 px-6 text-left">
                    <div className="font-ui-sm font-bold text-dark text-sm">{line.employeeName}</div>
