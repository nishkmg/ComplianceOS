        ...
        {TEMPLATES.map((t) => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              {t.recommended && (
                <span className="font-ui text-ui-2xs uppercase tracking-widest bg-dark text-white px-2 py-0.5 rounded-md">
                  Recommended
                </span>
              )}
            </div>
            <h3 className="font-ui text-lg font-bold text-on-surface mb-3">{t.name}</h3>
            <p className="font-ui text-ui-sm text-text-mid leading-relaxed flex-1">{t.desc}</p>
            {selectedTemplate === t.id && (
              <div className="mt-6 flex items-center gap-2 text-primary font-ui text-ui-xs uppercase tracking-widest font-bold">
