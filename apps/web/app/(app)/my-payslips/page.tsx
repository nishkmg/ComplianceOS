  ...
  return (
    <div className="space-y-0 text-left">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10 border-b-[0.5px] border-border-subtle pb-6">
        <div>
          <h1 className="font-display-xl text-display-xl text-dark mb-1">My Payslips</h1>
          <p className="font-ui-sm text-sm text-mid max-w-lg">Access your monthly compensation statements, tax deductions, and historical earning records.</p>
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center border border-border-subtle bg-white relative">
            <select 
              className="bg-transparent text-dark py-2.5 pl-4 pr-10 outline-none border-none cursor-pointer font-ui-sm text-sm"
              value={year}
      ...
      {/* Employee Meta */}
      <section className="bg-white border border-border-subtle mb-10 relative overflow-hidden shadow-sm p-8">
        <div className="absolute top-0 left-0 w-full h-[2px] bg-primary-container"></div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-1">Employee Name</p>
            <p className="font-ui-lg text-lg font-bold text-dark">Rahul Sharma</p>
          </div>
          <div>
            <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-1">Emp Code</p>
            <p className="font-mono text-lg font-medium text-dark">EMP-2024-001</p>
          </div>
          <div>
            <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-1">Department</p>
            <p className="font-ui-lg text-lg font-bold text-dark">Compliance</p>
          </div>
          <div>
            <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-1">UAN / PF Number</p>
      ...
      <div className="bg-white border border-border-subtle shadow-sm overflow-hidden">
        <div className="px-6 py-4 bg-section-muted border-b border-border-subtle">
          <h3 className="font-ui-md font-bold text-dark uppercase tracking-wider text-[11px] text-light">Compensation History</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-section-muted border-b-[0.5px] border-border-subtle">
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Month</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Gross Earnings (₹)</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Deductions (₹)</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Net Take-Home (₹)</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest">Status</th>
                <th className="py-3 px-6 font-ui-xs text-[10px] text-light uppercase tracking-widest text-right">Actions</th>
              </tr>
            </thead>
              ...
              {mockPayslips.map((p) => (
                <tr key={p.id} className="hover:bg-section-muted/30 transition-colors">
                  <td className="py-4 px-6 text-left">
                    <div className="font-ui-sm font-bold text-dark text-sm">{p.month}</div>
                  ...
                  <td className="py-4 px-6">
                    <span className="inline-block px-2 py-0.5 bg-success-bg text-success border border-green-200 text-[10px] font-bold uppercase rounded-sm">Paid</span>
                  </td>
