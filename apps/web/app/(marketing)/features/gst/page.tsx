      ...
      <main className="pt-32 pb-24">
        ...
        <section className="max-w-[1200px] mx-auto px-8 mb-space-64">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={s.label} className="bg-white border border-border-subtle border-t-2 border-t-primary-container p-6 shadow-sm text-left">
                <p className="font-ui-xs text-[10px] text-light uppercase tracking-widest mb-2 font-bold">{s.label}</p>
          ...
          {[
            ...
            <div key={m.title} className={`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${m.reversed ? 'lg:direction-rtl' : ''}`}>
              <div className={`text-left ${m.reversed ? 'lg:order-2' : ''}`}>
                <h3 className="font-display-xl text-display-xl text-dark mb-6">{m.title}</h3>
                <p className="font-ui-md text-ui-md text-mid leading-relaxed">{m.desc}</p>
              </div>
        ...
        <section className="bg-section-muted py-space-64">
          <div className="max-w-[1200px] mx-auto px-8">
            <h2 className="font-display-xl text-display-xl text-dark mb-16 text-center">Everything your GST module needs.</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <div key={f.title} className="bg-white border border-border-subtle p-8 shadow-sm border-t-2 border-t-primary-container text-left">
                  <Icon name={f.icon} className="text-primary-container text-3xl mb-4 block" />
                  <h3 className="font-ui-lg font-bold text-dark mb-3">{f.title}</h3>
                  <p className="font-ui-sm text-sm text-mid leading-relaxed">{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-space-96 px-8 text-center">
          <div className="max-w-2xl mx-auto">
            <h2 className="font-marketing-xl text-marketing-xl text-dark mb-6">Start generating compliant returns today.</h2>
            <p className="font-ui-md text-ui-md text-mid mb-10">No GST data entry. No CSV exports. No portal re-entry.</p>
