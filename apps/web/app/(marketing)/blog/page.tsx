    ...
    <div className="bg-page-bg text-on-surface min-h-screen">
      ...
      <main className="max-w-[1320px] mx-auto px-8">
        ...
        <section className="pt-space-128 pb-space-96">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 bg-white overflow-hidden border-[0.5px] border-border-subtle">
            ...
            <div className="flex flex-col justify-center p-12 text-left">
              ...
              <Link href={`/blog/${featured.slug}`} className="no-underline group">
                <h1 className="font-display-xl text-display-xl text-on-surface mb-4 group-hover:text-primary transition-colors">{featured.title}</h1>
              </Link>
              <p className="font-ui-md text-ui-md text-text-mid leading-relaxed mb-6">{featured.excerpt}</p>
              <div className="flex items-center justify-between">
                <span className="text-ui-xs text-text-light">{featured.author} · {featured.date}</span>
        ...
        <section className="border-b-[0.5px] border-border-subtle pb-space-48 mb-space-48">
          <div className="bg-section-muted p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-left">
              <h3 className="font-display-md text-display-md text-on-surface font-normal">Stay ahead of compliance changes.</h3>
              <p className="text-ui-sm text-ui-sm text-text-mid mt-1">Get our weekly newsletter straight to your inbox.</p>
            </div>
            <div className="flex gap-2">
              <input className="border-[0.5px] border-border-subtle bg-white px-4 py-3 font-ui-sm text-ui-sm w-full md:w-80 focus:outline-none focus:border-primary" placeholder="professional@firm.com" type="email" />
        ...
        <section className="pb-space-128">
          <div className="flex justify-between items-end mb-12 border-b-2 border-on-surface pb-4">
            ...
            <div className="hidden md:flex gap-6 font-ui-xs text-text-light uppercase tracking-widest">
              <button className="text-on-surface font-bold cursor-pointer border-none bg-transparent">All</button>
              <button className="hover:text-on-surface cursor-pointer border-none bg-transparent">Taxation</button>
              <button className="hover:text-on-surface cursor-pointer border-none bg-transparent">Audit</button>
              <button className="hover:text-on-surface cursor-pointer border-none bg-transparent">Payroll</button>
              <button className="hover:text-on-surface cursor-pointer border-none bg-transparent">SME Guide</button>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-16">
            {posts.slice(1).map((post) => (
              <article key={post.slug} className="group text-left">
                <div className="aspect-[16/10] overflow-hidden mb-6 bg-white border-[0.5px] border-border-subtle">
                ...
                <Link href={`/blog/${post.slug}`} className="no-underline">
                  <h3 className="font-display-lg text-display-lg text-on-surface mt-2 mb-3 group-hover:text-primary transition-colors">{post.title}</h3>
                </Link>
                <p className="font-ui-sm text-ui-sm text-text-mid mb-4 leading-relaxed">{post.excerpt}</p>
                <span className="text-ui-xs text-text-light">{post.author} · {post.date}</span>
              </article>
            ))}
          </div>
        </section>
