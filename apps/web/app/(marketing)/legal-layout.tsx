      ...
      <main id="main-content">
        <article className="py-16 md:py-24">
          <div className="marketing-container max-w-[760px]">
            ...
            {lastUpdated && <p className="font-ui text-ui-sm text-light mb-8">Last updated: {lastUpdated}</p>}
            
            <nav className="mb-10 font-ui text-ui-sm space-y-2">
              {sections.map((s) => (
                <a key={s.id} href={`#${s.id}`} className="block text-amber hover:underline">{s.title}</a>
              ))}
            </nav>
            
            <div className="font-ui text-ui-lg text-dark leading-relaxed space-y-6">
              {children}
            </div>
          </div>
        </article>
