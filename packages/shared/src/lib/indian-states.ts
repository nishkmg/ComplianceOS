...
export const INDIAN_STATES: IndianState[] = [
  ...
  { code: "26", name: "Dadra & Nagar Haveli", type: "ut" },
...
const STATE_MAP = new Map(INDIAN_STATES.map((s) => [s.code, s]));

export function getStateName(code: string): string | undefined {
  return STATE_MAP.get(code)?.name;
}

export function isUT(code: string): boolean {
  return STATE_MAP.get(code)?.type === "ut";
}

export function stateCodeToGstPrefix(code: string): string {
  return `IN-${code}`;
}
