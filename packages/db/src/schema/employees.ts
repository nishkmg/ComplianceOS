import {
  pgTable, uuid, text, date, timestamp, boolean, jsonb,
  uniqueIndex, index,
...
export const taxPreferences = pgTable("tax_preferences", {
  ...
  uniqueIndex("tax_preferences_tenant_employee_fy_unique").on(
    table.tenantId, table.employeeId, table.financialYear
  ),
]);
